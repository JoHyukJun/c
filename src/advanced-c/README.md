...
### lookup table
    ...
    - 함수 포인터 배열로 구현
    - 바이 코드 값에 따른 switch-case 없이 처리 함수 즉시 선택
    - vm 의 성능을 높이기 위한 필수요소

## 07-multi-thread

### thread

    - 하나의 process 는 최소 1개 이상의 thread 를 가짐
    - thread 는 개별화된 흐름과 전용 스택을 갖는 실행의 단위
    - 모든 thread 는 자신이 속한 process 의 가상 메모리 공간을 공유
...
### tread sync
    ...
    - 동기화 객체를 이용해 구현
        ...
        - mutex
        - semaphore
    - 스레드의 실행은 동시성이 보장되어야 함
    - 전역변수처럼 여러 스레드가 동시 접근할 수 있는 대상에 대해서는 반드시 동기화 적용
    - 동기화는 특정 구간 코드에 대한 독점적 실행을 보장하는 것

### debuging

    - 스레드는 개별화된 흐름을 가지며 일반적인 position break point 로는 디버깅 한계 존재
    - 특졍 변수나 자료구조에 대한 변화를 추적할 수 있는 옵저버 방식으로 디버깅
    - 콘솔 출력은 그 자체로 동기화에 영향을 줄 수 있으므로 주의

## 08-libaray

### 정적/동적 라이브러리

    - 라이브러리는 함수만 따로 모은 실행 파일
    - 함수라고 표현하다 API라고 부른는 경우가 많고 특정 환경에 맞춘 종합적인 API 묶음을 SDK라고 함
    - 정적 라이브러리
        - 실행 파일과 link time 에 결합되어 한 실행파일로 합성
        - 최종 바이너리는 .lib 파일
        - lib 파일에 구현되어 있는 함수를 사용하는 exe 는 lib 파일을 반드시 link
        - .c, .h, .lib 등 하나로 구성
    - 동적 라이브러리
        - 실행 파일과 link time 에 결합도지 않고 독립적인 실행파일 형태로 생성되며 실행 파일이 실행 될 때 동적으로 결합
        - 최종 바이너리는 .dll 파일
        - compile time (.h)
        - link time (.lib)
        - run time (.dll)
...
### shell
    ...
    - 다른 프로그램을 실행
    - 특정 프로세스가 스스로 새 프로세스를 생성할 경우 권한이 그대로 복제

### stack overflow

    - 자동변수로 관리되는 지역변수나 함수 매개변수는 스택 영역을 사용
    - 스택에서 함수가 반환할 때 되돌아갈 메모리의 주소인 return address 가 포함되어 있으며 변조 가능
